...
function check_size(){
	$('.item.year').imagesLoaded(function(){
		...
		if($('html, body').width()>=626 && $('html, body').width()<=1184){
			$('article.work .part .item:nth-child(2)').not(".count1").css("top",work2_h);
		}else{
			$("article.work .part .item:nth-child(2)").not(".count1").removeAttr('style');
		}
	});
	...
	$('article.index .item').imagesLoaded(function(){
		...
		if($('html, body').width()<625){
			$('article.index .item.w1.c:nth-last-of-type(2)').not(".count1").css("top",p_h);
		}else{
			$("article.index .item.w1.c:nth-last-of-type(2)").not(".count1").removeAttr('style');
		}
	});
};
//--------------   SLIDE_SHOW   --------------//
function slide(){
	...
	$('.fotorama').on('fotorama:ready ' +  'fotorama:load ',function (e, fotorama, extra) {
