...
class Works_model extends CI_Model {
    ...
    public function rs($year="",$typeid=""){
        $result = array();
        $search = "";
        if($typeid!=""){
            $search = "AND typeid='{$typeid}'";
        }
        if($year!=""){
            $year = explode(",",$year);
            foreach($year as $y){
                $result[$y] = array();
                $sql = "SELECT {$this->title} as title,works.* FROM works WHERE isdelete='0' AND isshow='1' AND yy='{$y}' {$search} ORDER BY sort";
                $query = $this->db->query($sql);
                foreach($query->result_array() as $row){
                    $row['type'] = $this->get_type($row['typeid']);
                    array_push($result[$y],$row);
                }
            }
        }
        // print_r($result);exit;
    ...
    public function get_year($start=0,$pagesize=2,$tag=""){
        $result = array();
        $search = "";
        if($tag!=""){
            $search = "AND typeid = '{$tag}'";
        }
        $sql = "SELECT yy FROM works WHERE isdelete='0' AND isshow='1' {$search} GROUP BY yy ORDER BY yy DESC limit {$start},{$pagesize}";
        $query = $this->db->query($sql);
    ...
    public function index_works(){
        $sql="SELECT {$this->title} as title,works.* FROM works WHERE isdelete='0' AND isshow = '1' ORDER BY sort limit 0,4";
        $query = $this->db->query($sql);
        $row = $query->result_array();
        $row[0]['imgs'] = $this->get_imgs($row[0]['workid']);
        return $row;
    }
