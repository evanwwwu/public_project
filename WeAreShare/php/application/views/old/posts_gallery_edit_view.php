  ...
  <div class="pure-g-r " id="layout">
   ...
   <div class="pure-u" id="main">
    <div class="content">
     ...
     <form id="frm" class="pure-form pure-form-stacked">
      ...
      <fieldset>
       ...
       <div style="">
        ...
        <div style="display:inline-block;"><a href="<?PHP echo SITE_URL?>gallery/<?PHP echo $data['post_name']?>?preview=1" target="_blank" class="pure-button pure-button-primary">預覽</a></div>
        <div style="display:inline-block;"><a href="<?PHP echo ADMIN_URL?>posts_gallery/images/<?PHP echo $data['ID']?>" class="pure-button pure-button-primary">編輯照片</a></div>
        <div style="display:inline-block;"><a title="DELETE" class="pure-button error" onclick="del(<?PHP echo $data['ID']?>)" >DELETE</a></div>
        <div style="display:inline-block;"><a href="<?PHP echo ADMIN_URL?>posts_gallery" class="pure-button">BACK</a></div>
        </div>
       ...
       <span class="term" id="term_<?PHP echo $term['term_id']?>">
       <input name="term[]" value="<?PHP echo $term['term_id']?>" type="checkbox" <?PHP echo (in_array($term['term_id'],$data['terms'])?"checked":"")?>> 
         <?PHP echo $term['name']?> 
        <a class ="new_term" href="javascript:void(0)" onClick="delete_tag(<?PHP echo $term['term_id']?>)"><i class="icon-trash"></i></a>
       </span>
       <?PHP endforeach;?>
       <div id = "new_term">
       <label>新增類別</label>
     ...
     <label>推薦</label>
     <div>
        <a href="javascript:void(0);" class="pure-button rec "event="post">推薦文章</a>
<!--         <a href="javascript:void(0);" class="pure-button rec "event="event">推薦派對</a> -->
     </div>
     <div id="rec_panel">
...
$(function () {
    var admin_active = '<?PHP echo $_SESSION[ADMIN_ACTIVE] ?>';
    if(admin_active!='1'){
      $('#new_term').hide();
      $('.new_term').hide();
    }
    ...
    $('#cover_file').fileupload({
        ...
        url: '<?PHP echo ADMIN_URL?>file/upload02/cover_file',
    ...
    $('#gallery').sortable({
        ...
        stop: function (event, ui) {
            ...
            console.log(q);
            $.post('<?PHP echo ADMIN_URL?>posts_gallery/gallery_sort', q)
        }
    });
    ...
    $('#frm').ajaxForm({
      beforeSubmit:function(){change=0;},
        url: '<?PHP echo ADMIN_URL?>posts_gallery/save',
        type: 'post',
        dataType: 'script'
    });
...
function recommend_autocomplte(obj) {
    obj.bind("keydown", function (event) {
        ...
        .autocomplete({
            source: function (request, response) {
                $.getJSON("<?PHP echo ADMIN_URL . 'posts/search_posts/' . $data['ID']?>", {
                    term: extractLast(request.term)
                }, response);
            },
            search: function () {
...
function delete_img(obj) {
    ...
    target.empty();
    $.post('<?PHP echo SITE_URL?>admin/posts/delete_cover', 'id=' + $('#id').val(), null, 'script');
}
...
function delete_img_gallery(obj, filename) {
    ...
    var target = obj.parent();
    target.remove();
    $.post('<?PHP echo SITE_URL?>admin/posts/delete_gallery', 'id=' + $('#id').val() + '&filename=' + filename, null, 'script');
}

function delete_tag(id) {
    if (!confirm('確定刪除類別?')) return;
    $.post('<?PHP echo SITE_URL?>admin/posts/delete_tag', 'id=' + id, function(){$("#term_"+id).remove();}, 'script');
}
function delete_recommend(id){
  if (!confirm('確定刪除?')) return;
  $.post('<?PHP echo SITE_URL?>admin/posts/delete_recommend', 'id=' + id, function(){$('#rec_'+id).remove();}, 'script');
}
function del(id){
  if (!confirm('Delete?')) return;
  $.post('<?PHP echo ADMIN_URL?>posts_gallery/del/0','id='+id,function(){
    location = '<?PHP echo ADMIN_URL?>posts_gallery';
  },'script');
}
</script>
</body>
