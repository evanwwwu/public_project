	...
	<div class="pure-u" id="main">
		<div class="content">
			...
			<form class="pure-form">
            	<div class="right"><a onclick="change=0;" class="pure-button notice" href="<?PHP echo ADMIN_URL?>brand/edit" title="新增">新增</a></div>
            </form>
            ...
            <ul class="pure-paginator">
                    ...
                    <li><a onclick="change=0;" class="pure-button double_prev"  href="<?PHP echo ADMIN_URL . 'brand/?page=1'.$data["search_url"]?>">&#171;</a></li>
                        <li><a class="pure-button prev" href="#" onclick="change=0;page(0)" >&#60;</a></li>

                    <?PHP if($data['current']<=5 ):?>
                            <?PHP for($x=0,$i=1;$i<=$data['pages']&&$x<10;$i++,$x++):?>
                        <li><a onclick="change=0;" class="pure-button <?PHP echo ($i==$data['current'])?"pure-button-active":""?>" href="<?PHP echo ADMIN_URL . 'brand/?page='.$i.$data["search_url"]?>"><?PHP echo $i;?></a></li>
                        <?PHP endfor;?>
                ...
                <li><a onclick="change=0;" class="pure-button double_next" href="<?PHP echo ADMIN_URL . 'brand/?page='.$data['pages'].$data["search_url"]?>">&#187;</a></li>
            </ul>
            <?PHP endif;?>
            <br>  
            <table class="pure-table pure-table-bordered" width="100%">
                ...
                <tbody>
                    ...
                    <tr id="row_<?PHP echo $row['ID']?>">
                        ...
                        <td><a onclick="change=0;" href="<?PHP echo SITE_URL?>/brand/letter/<?PHP echo $row['letter']?>" target="_blank"><?PHP echo $row['letter']?></a></td>
                        <td><a onclick="change=0;" href="<?PHP echo SITE_URL?>brand/<?PHP echo $row['post_name']?>?preview=1" target="_blank" ><?PHP echo $row['post_title']?></a></td>
                        <td>
                        ...
                        <td>
                            <a onclick="change=0;" class="pure-button" title="編輯" href="<?PHP echo ADMIN_URL . 'brand/edit/' .$row['ID']?>"><i class="icon-pencil"></i></a> 
...
function publish(obj){
	...
	$.post('<?PHP echo ADMIN_URL?>brand/publish',q,null,'script');
}

function del(id){
	if (!confirm('Delete?')) return;
	$.post('<?PHP echo ADMIN_URL?>brand/del','id='+id,null,'script');
}
function page(act){
    ...
    if(act=='1'){
        cur++;
        if(cur<pages){
            url = "<?PHP echo ADMIN_URL . 'brand/?page="+cur+"'.$data["search_url"]?>";
        }
        else{
            url = "<?PHP echo ADMIN_URL . 'brand/?page="+pages+"'.$data["search_url"]?>";
        }
    }
    else{
        cur--;
        if(cur>1){
            ...
            url = "<?PHP echo ADMIN_URL . 'brand/?page="+1+"'.$data["search_url"]?>";
        }
