  ...
  <div class="pure-g-r " id="layout">
   ...
   <div class="pure-u" id="main">
    <div class="content">
     ...
     <form id="frm" class="pure-form pure-form-stacked">
      ...
      <fieldset>
       ...
       <div style="">
        ...
        <div style="display:inline-block;"><a href="<?PHP echo SITE_URL?>brand/<?PHP echo $data['post_name']?>?preview=1" target="_blank" class="pure-button pure-button-primary">預覽</a></div>
        <div style="display:inline-block;"><a title="DELETE" class="pure-button error" onclick="del(<?PHP echo $data['ID']?>)" >DELETE</a></div>
        <div style="display:inline-block;"><a href="<?PHP echo ADMIN_URL?>brand" class="pure-button">BACK</a></div>
        </div>
     ...
     <label>推薦</label>
     <div>
        <a href="javascript:void(0);" class="pure-button rec "event="brand">推薦品牌</a>
     </div>
...
$(function () {
    ...
    $('#gallery').sortable({
        ...
        stop: function (event, ui) {
            ...
            console.log(q);
            $.post('<?PHP echo ADMIN_URL?>brand/gallery_sort', q)
        }
    });
    ...
    $('#frm').ajaxForm({
        ...
        url: '<?PHP echo ADMIN_URL?>brand/save',
        type: 'post',
        dataType: 'script'
    });
    ...
    $('.add2rec').bind('click', function () {
        if ($('#rec_id').val() == 0) {
            ...
            $.post('<?PHP echo ADMIN_URL?>brand/add2rec/<?PHP echo $data['ID']?>/' + $('#rec_id').val(), function (data) {
    ...
    obj.bind("keydown", function (event) {
        ...
        .autocomplete({
            source: function (request, response) {
                $.getJSON("<?PHP echo ADMIN_URL . 'brand/search_tag'?>", {
...
function recommend_autocomplte(obj) {
    obj.bind("keydown", function (event) {
        ...
        .autocomplete({
            source: function (request, response) {
                $.getJSON("<?PHP echo ADMIN_URL . 'brand/search_posts/' . $data['ID']?>", {
                    term: extractLast(request.term)
                }, response);
            },
            search: function () {
...
function location_autocomplte(obj) {
    obj.bind("keydown", function (event) {
        ...
        .autocomplete({
            source: function (request, response) {
                $.getJSON("<?PHP echo ADMIN_URL . 'brand/search_location/' . $data['ID']?>", {
                    term: extractLast(request.term)
                }, response);
            },
            search: function () {
...
function delete_img(obj) {
    ...
    target.empty();
    $.post('<?PHP echo SITE_URL?>admin/brand/delete_cover', 'id=' + $('#id').val(), null, 'script');
}
...
function delete_img_gallery(obj, filename) {
    ...
    var target = obj.parent();
    target.remove();
    $.post('<?PHP echo SITE_URL?>admin/brand/delete_gallery', 'id=' + $('#id').val() + '&filename=' + filename, null, 'script');
}

function delete_tag(id) {
    if (!confirm('確定刪除類別?')) return;
    $.post('<?PHP echo SITE_URL?>admin/brand/delete_tag', 'id=' + id, null, 'script');
}
function delete_recommend(id){
  if (!confirm('確定刪除?')) return;
  $.post('<?PHP echo SITE_URL?>admin/brand/delete_recommend', 'id=' + id, function(){$('#rec_'+id).remove();}, 'script');
}
function del(id){
  if (!confirm('Delete?')) return;
  $.post('<?PHP echo ADMIN_URL?>brand/del/0','id='+id,function(){
    location = '<?PHP echo ADMIN_URL?>brand';
  },'script');
}
</script>
</body>
