     ...
     <form id="frm" class="pure-form pure-form-stacked">
      ...
      <fieldset>
       ...
       <div style="">
        ...
        <div style="display:inline-block;"><a href="<?PHP echo SITE_URL?>event/<?PHP echo $data['post_name']?>?preview=1" target="_blank" class="pure-button pure-button-primary">預覽</a></div>
        <div style="display:inline-block;"><a href="<?PHP echo ADMIN_URL?>event/images/<?PHP echo $data['ID']?>" class="pure-button pure-button-primary">編輯照片</a></div>
        <div style="display:inline-block;"><a title="DELETE" class="pure-button error" onclick="del(<?PHP echo $data['ID']?>)" >DELETE</a></div>
        <div style="display:inline-block;"><a href="<?PHP echo ADMIN_URL?>event" class="pure-button">BACK</a></div>
        </div>
...
$(function () {
    ...
    $('#gallery').sortable({
        ...
        stop: function (event, ui) {
            ...
            console.log(q);
            $.post('<?PHP echo ADMIN_URL?>event/gallery_sort', q)
        }
    });
    ...
    $('#frm').ajaxForm({
      beforeSerialize:function(){change=0;},
...
function recommend_autocomplte(obj) {
    obj.bind("keydown", function (event) {
        ...
        .autocomplete({
            source: function (request, response) {
                $.getJSON("<?PHP echo ADMIN_URL . 'event/search_posts/' . $data['ID']?>", {
                    term: extractLast(request.term)
                }, response);
            },
            search: function () {
...
function location_autocomplte(obj) {
    obj.bind("keydown", function (event) {
        ...
        .autocomplete({
            source: function (request, response) {
                $.getJSON("<?PHP echo ADMIN_URL . 'event/search_location/' . $data['ID']?>", {
                    term: extractLast(request.term)
                }, response);
            },
            search: function () {
...
function delete_img(obj) {
    ...
    target.empty();
    $.post('<?PHP echo SITE_URL?>admin/event/delete_cover', 'id=' + $('#id').val(), null, 'script');
}
...
function delete_img_gallery(obj, filename) {
    ...
    var target = obj.parent();
    target.remove();
    $.post('<?PHP echo SITE_URL?>admin/event/delete_gallery', 'id=' + $('#id').val() + '&filename=' + filename, null, 'script');
}

function delete_tag(id) {
    if (!confirm('確定刪除類別?')) return;
    $.post('<?PHP echo SITE_URL?>admin/event/delete_tag', 'id=' + id, null, 'script');
}
function delete_recommend(id){
  if (!confirm('確定刪除?')) return;
  $.post('<?PHP echo SITE_URL?>admin/event/delete_recommend', 'id=' + id, function(){$('#rec_'+id).remove();}, 'script');
}
function del(id){
  if (!confirm('Delete?')) return;
  $.post('<?PHP echo ADMIN_URL?>event/del/0','id='+id,function(){
    location = '<?PHP echo ADMIN_URL?>event';
  },'script');
}
</script>
</body>
