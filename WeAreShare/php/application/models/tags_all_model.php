...
class Tags_all_model extends CI_Model {
    public function __construct(){
        parent::__construct();
   //     $this->load->helper('url');
    }
	
	public function rs($current=1,$pagesize=20){
		$result = array();
		$search = '';
		$search_url = '';
		if (isset($this->get['search_key']) and $this->get['search_key']!=''){
			$search .= " and (name like '%{$this->get['search_key']}%')";
			$search_url .= '&search_key='.$this->get['search_key'];
		}

		$sql = "SELECT * FROM terms WHERE term_group = '1' {$search} ORDER BY term_id DESC";
	...
	public function save($id=0){
			...
			$sql = "select * from terms where LOWER(name)='" . addslashes(strtolower($term))."' and term_group=0 {$search}";
			$query = $this->db->query($sql);
			if ($query->num_rows()>0){
				echo "";
			}
				...
				if($id==0){
					$sql = "insert into terms values(NULL,'".addslashes($term)."','".addslashes(str_replace(' ', '-', $term))."',1,0)";
					$this->db->query($sql);
					echo "true";
				}
				else{
