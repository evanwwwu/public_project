...
class Society_model extends CI_Model {
	...
	public function rs($start=0,$pagesize=6){
		$result = array();
		$sql = "SELECT * FROM posts a, postmeta c WHERE a.id=c.post_id AND c.meta_key='letter' AND (post_type='brand' or post_type='people') AND post_status='publish' GROUP BY id  ORDER BY c.meta_value";
	...
	public function rs_letter($chr='',$start=0,$pagesize=6){
		...
		$sql = "select a.* from posts a,postmeta b where a.ID=b.post_id and (post_type='brand' or post_type='people'){$search_chr} and b.meta_key='letter' and post_status='publish' order by post_title";
		$query = $this->db->query($sql);
		$result['total_posts'] = $query->num_rows;
		$data = array();
		$sql .= " limit {$start},{$pagesize}";
	...
	public function tags($tag_id,$start=0,$pagesize=6){
		$sql = "select ID from posts a,term_relationships b where a.ID=b.object_id and b.term_taxonomy_id='{$tag_id}' and (post_type='brand' or post_type='people') and post_date<='".date('Y-m-d H:i:s')."' and post_status='publish' order by post_date desc,ID desc";
				$query = $this->db->query($sql);
				$result['total_posts'] = $query->num_rows;
				$data = array();
				$sql = "select * from posts a,term_relationships b where a.ID=b.object_id and b.term_taxonomy_id='{$tag_id}' and (post_type='brand' or post_type='people') and post_date<='".date('Y-m-d H:i:s')."' and post_status='publish' order by post_date desc,ID desc limit {$start},{$pagesize}";
				$query = $this->db->query($sql);
	...
	public function row($title){
		...
		$sql = "select * from posts where post_title='{$title}' {$search_status} and (post_type='brand' or post_type='people')";
		$query = $this->db->query($sql);
		if ($query->num_rows()>0){
