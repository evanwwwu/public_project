...
class Calendar_model extends CI_Model {
	...
	public function rs($author_id=0,$tag_id=0,$start=0,$pagesize=6){
		...
		if($author_id==0){
			if ($tag_id==0){
				$sql = "select ID from posts a,postmeta b where a.ID=b.post_id and b.meta_key='end_date' and a.post_type='calendar' and b.meta_value>'".date('Y-m-d')."' and a.post_status='publish' and a.post_date<='".date('Y-m-d H:i:s')."' order by b.meta_value,a.ID desc";
				$query = $this->db->query($sql);
				$result['total_posts'] = $query->num_rows;
				$data = array();
				$sql = "select * from posts a,postmeta b where a.ID=b.post_id and b.meta_key='end_date' and a.post_type='calendar' and b.meta_value>'".date('Y-m-d')."' and a.post_status='publish' and a.post_date<='".date('Y-m-d H:i:s')."' order by b.meta_value,a.ID desc limit {$start},{$pagesize}";
				$query = $this->db->query($sql);
			}
			else{
				$sql = "select ID from posts a,term_relationships b where a.ID=b.object_id and b.term_taxonomy_id='{$tag_id}' and post_type='calendar' and post_date<='".date('Y-m-d H:i:s')."' and post_status='publish' order by post_date desc,ID desc";
				$query = $this->db->query($sql);
				$result['total_posts'] = $query->num_rows;
				$data = array();
				$sql = "select * from posts a,term_relationships b where a.ID=b.object_id and b.term_taxonomy_id='{$tag_id}' and post_type='calendar' and post_date<='".date('Y-m-d H:i:s')."' and post_status='publish' order by post_date desc,ID desc limit {$start},{$pagesize}";
				$query = $this->db->query($sql);
			}
		}
		else{
			$sql = "select ID from posts  where post_author= {$author_id} and post_type='calendar' and post_date<='".date('Y-m-d H:i:s')."' and post_status='publish' order by post_date desc,ID desc";
			$query = $this->db->query($sql);
			$result['total_posts'] = $query->num_rows;
			$data = array();
			$sql = "select * from posts  where post_author= {$author_id} and post_type='calendar' and post_date<='".date('Y-m-d H:i:s')."' and post_status='publish' order by post_date desc,ID desc limit {$start},{$pagesize}";
			$query = $this->db->query($sql);
		}
	...
	public function search_month($date,$start=0,$pagesize=6){
		$result = array();
		$sql = "select ID from posts where post_type='calendar' and post_status='publish' and post_date like '{$date}%' order by post_date desc,ID desc";
		$query = $this->db->query($sql);
		$result['total_posts'] = $query->num_rows;
		$data = array();
		$sql = "select * from posts where post_type='calendar' and post_status='publish' and post_date like '{$date}%' order by post_date desc,ID desc limit {$start},{$pagesize}";
		$query = $this->db->query($sql);
	...
	public function row($title){
		...
		$sql = "select * from posts where post_name='{$title}' {$search_status} and post_type='calendar'";
		$query = $this->db->query($sql);
		if ($query->num_rows()>0){
	...
	public function get_ym_menu($yy){
		...
		for ($i=1;$i<=12;$i++){
			$sql = "SELECT YEAR(post_date) AS yy,MONTH(post_date) AS mm,COUNT(*) AS posts 
					FROM posts WHERE post_type='calendar' 
	...
	public function get_recommend($id){
		...
		if (strlen($search_str) > 0){
			$sql = "select * from posts where ID in({$search_str}) and post_status='publish' and post_type='calendar'";
			$query = $this->db->query($sql);
		...
		if (strlen($search_str) > 0){
			$sql = "select * from posts where ID in({$search_str}) and post_status='publish' and post_type='calendar' order by rand()";
			$query = $this->db->query($sql);
	...
	private function get_prev_post($date,$id){
		if ($_SESSION['tag_from'] == ''){
			$sql = "select post_title,post_name from posts where post_status='publish' and post_type='calendar' and post_date>'{$date}' and ID<>{$id} order by post_date limit 0,1";
			$query = $this->db->query($sql);
			if ($query->num_rows==0){
				$sql = "select post_title,post_name from posts where post_status='publish' and post_type='calendar' and post_date='{$date}' and ID<>{$id} order by post_date limit 0,1";
				$query = $this->db->query($sql);
				if ($query->num_rows>0){
					$sql = "select post_title,post_name from posts where post_status='publish' and post_type='calendar' and ID>{$id} order by ID limit 0,1";
				}
			}
		}
		else{
	...
	private function get_next_post($date,$id){
		if ($_SESSION['tag_from'] == ''){
			$sql = "select post_title,post_name from posts where post_status='publish' and post_type='calendar' and post_date<'{$date}' and ID<>{$id} order by post_date desc limit 0,1";
			$query = $this->db->query($sql);
			if ($query->num_rows==0){
				$sql = "select post_title,post_name from posts where post_status='publish' and post_type='calendar' and post_date='{$date}' and ID<>{$id} order by post_date desc limit 0,1";
				$query = $this->db->query($sql);
				if ($query->num_rows>0){
					$sql = "select post_title,post_name from posts where post_status='publish' and post_type='calendar' and ID<{$id} order by ID desc limit 0,1";
				}
			}
		}
		else{
