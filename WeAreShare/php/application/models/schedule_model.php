...
class Schedule_model extends CI_Model {
	...
	public function rs($author_id=0,$tag_id=0,$start=0,$pagesize=6){
		...
		if($author_id==0){
			if ($tag_id==0){
				$sql = "select ID from posts where ((post_type='event' or post_type='calendar') or post_type='calendar') and post_status='publish' and post_date<='".date('Y-m-d H:i:s')."' order by post_date desc,ID desc";
				$query = $this->db->query($sql);
				$result['total_posts'] = $query->num_rows;
				$data = array();
				$sql = "select * from posts where (post_type='event' or post_type='calendar') and post_status='publish' and post_date<='".date('Y-m-d H:i:s')."' order by post_date desc,ID desc limit {$start},{$pagesize}";
				$query = $this->db->query($sql);
			}
			else{
				$sql = "select ID from posts a,term_relationships b where a.ID=b.object_id and b.term_taxonomy_id='{$tag_id}' and (post_type='event' or post_type='calendar') and post_date<='".date('Y-m-d H:i:s')."' and post_status='publish' order by post_date desc,ID desc";
				$query = $this->db->query($sql);
				$result['total_posts'] = $query->num_rows;
				$data = array();
				$sql = "select * from posts a,term_relationships b where a.ID=b.object_id and b.term_taxonomy_id='{$tag_id}' and (post_type='event' or post_type='calendar') and post_date<='".date('Y-m-d H:i:s')."' and post_status='publish' order by post_date desc,ID desc limit {$start},{$pagesize}";
				$query = $this->db->query($sql);
			}
		}
		else{
			$sql = "select ID from posts  where post_author= {$author_id} and (post_type='event' or post_type='calendar') and post_date<='".date('Y-m-d H:i:s')."' and post_status='publish' order by post_date desc,ID desc";
			$query = $this->db->query($sql);
			$result['total_posts'] = $query->num_rows;
			$data = array();
			$sql = "select * from posts  where post_author= {$author_id} and (post_type='event' or post_type='calendar') and post_date<='".date('Y-m-d H:i:s')."' and post_status='publish' order by post_date desc,ID desc limit {$start},{$pagesize}";
			$query = $this->db->query($sql);
		}
		...
		foreach($query->result_array() as $row){
			...
			$row['start_date'] = $this->get_start_date($row['ID']);
			$row['end_date'] = $this->get_end_date($row['ID']);
			array_push($data,$row);
		}
	...
	public function search_month($date,$start=0,$pagesize=6){
		$result = array();
		$sql = "select ID from posts where (post_type='event' or post_type='calendar') and post_status='publish' and post_date like '{$date}%' order by post_date desc,ID desc";
		$query = $this->db->query($sql);
		$result['total_posts'] = $query->num_rows;
		$data = array();
		$sql = "select * from posts where (post_type='event' or post_type='calendar') and post_status='publish' and post_date like '{$date}%' order by post_date desc,ID desc limit {$start},{$pagesize}";
		$query = $this->db->query($sql);
	...
	public function get_start_date($id){
		$sql = "select * from postmeta where meta_key='start_date' and post_id='{$id}'";
		$query = $this->db->query($sql);
		if ($query->num_rows()>0){
			$row = $query->row_array();
	...
	public function get_end_date($id){
		$sql = "select * from postmeta where meta_key='end_date' and post_id='{$id}'";
		$query = $this->db->query($sql);
		if ($query->num_rows()>0){
			$row = $query->row_array();
			return $row['meta_value'];
	...
	public function row($title){
		...
		$sql = "select * from posts where post_name='{$title}' {$search_status} and (post_type='event' or post_type='calendar')";
		$query = $this->db->query($sql);
		if ($query->num_rows()>0){
			...
			foreach($tags as $tag){
				$result['post_content'] = str_replace($tag['name'],'<a href="'.SITE_URL.'event/tag/'.$tag['slug'].'" style="color:#ed1c24">'.$tag['name'].'</a>',$result['post_content']);
			}
	...
	public function get_ym_menu($yy){
		$result = array();
		$chinese_month = explode(',',',一月,二月,三月,四月,五月,六月,七月,八月,九月,十月,十一月,十二月');
		
		for ($i=1;$i<=12;$i++){
			$sql = "SELECT YEAR(post_date) AS yy,MONTH(post_date) AS mm,COUNT(*) AS posts 
					FROM posts WHERE (post_type='event' or post_type='calendar') 
					AND post_status='publish' 
					AND YEAR(post_date)={$yy} 
					AND MONTH(post_date)={$i} 
					GROUP BY YEAR(post_date),MONTH(post_date)";
			$query = $this->db->query($sql);
			if ($query->num_rows==0){
				array_push($result,array($chinese_month[$i],0));
			}
			else{
				$row = $query->row_array();
				array_push($result,array($chinese_month[$i],$row['posts']));
			}
		}
		$result = array('yy'=>$yy,'mm'=>$result);
		//print_r($result);exit;
		return $result;
	}
	...
	public function get_recommend($id){
		...
		if (strlen($search_str) > 0){
			$sql = "select * from posts where ID in({$search_str}) and post_status='publish' and (post_type='event' or post_type='calendar')";
			$query = $this->db->query($sql);
		...
		if (strlen($search_str) > 0){
			$sql = "select * from posts where ID in({$search_str}) and post_status='publish' and (post_type='event' or post_type='calendar') order by rand()";
			$query = $this->db->query($sql);
	...
	private function get_prev_post($date,$id){
		if ($_SESSION['tag_from'] == ''){
			$sql = "select post_title,post_name from posts where post_status='publish' and (post_type='event' or post_type='calendar') and post_date>'{$date}' and ID<>{$id} order by post_date limit 0,1";
			$query = $this->db->query($sql);
			if ($query->num_rows==0){
				$sql = "select post_title,post_name from posts where post_status='publish' and (post_type='event' or post_type='calendar') and post_date='{$date}' and ID<>{$id} order by post_date limit 0,1";
				$query = $this->db->query($sql);
				if ($query->num_rows>0){
					$sql = "select post_title,post_name from posts where post_status='publish' and (post_type='event' or post_type='calendar') and ID>{$id} order by ID limit 0,1";
				}
			}
		}
		else{
			$sql = "select post_title from posts a,term_relationships b 
					...
					and post_status='publish' 
					and (post_type='event' or post_type='calendar') 
					and post_date>'{$date}' 
	...
	private function get_next_post($date,$id){
		if ($_SESSION['tag_from'] == ''){
			$sql = "select post_title,post_name from posts where post_status='publish' and (post_type='event' or post_type='calendar') and post_date<'{$date}' and ID<>{$id} order by post_date desc limit 0,1";
			$query = $this->db->query($sql);
			if ($query->num_rows==0){
				$sql = "select post_title,post_name from posts where post_status='publish' and (post_type='event' or post_type='calendar') and post_date='{$date}' and ID<>{$id} order by post_date desc limit 0,1";
				$query = $this->db->query($sql);
				if ($query->num_rows>0){
					$sql = "select post_title,post_name from posts where post_status='publish' and (post_type='event' or post_type='calendar') and ID<{$id} order by ID desc limit 0,1";
				}
			}
		}
		else{
	...
	public function get_location($id){
		...
		$result = array();
		foreach($query->result_array() as $row){
			$html = '<a href="'.SITE_URL.'search?key='.$row['name'].'">'.$row['name'].'</a>';
