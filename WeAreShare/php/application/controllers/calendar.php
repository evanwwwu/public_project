...
class Calendar extends CI_Controller {
	...
	public function index(){
		...
		$this->data['posts'] = $this->calendar_model->rs();
		...
		$this->load->view('calendar_list_view',$this->data);
	}
	...
	public function month($yymm=''){
		...
		$this->data['meta']['brief'] = $meat[1];
		$this->data['meta']['keyword'] = $meat[2];
		$this->data['meta']['url'] = SITE_URL . 'calendar/month'.$yymm;
		$this->data['meta']['cover_img'] = 'FB.jpg';
		$this->data = $this->global_model->site_init(1,$this->data);
		...
		$this->data['posts'] = $this->calendar_model->search_month($yymm);
		$this->load->view('calendar_list_view',$this->data);
	}
	...
	public function posts($title=''){
		...
		$this->data['data'] = $this->calendar_model->row($title);
		...
		$this->data['meta']['url'] = SITE_URL . 'calendar/' .$this->data['data']['post_name'];
		$this->data['meta']['cover_img'] = $this->data['data']['cover_img'];
		$this->data = $this->global_model->site_init(1,$this->data);
		$this->load->view('calendar_detail_view',$this->data);
	}

	public function recommend($title=''){
		$data = $this->calendar_model->row($title);
		$data = $this->calendar_model->get_recommend($data['ID']);
		$result = array();
		foreach($data as $row){
			$item = array();
			$item['url'] = SITE_URL . 'calendar/' . $row['post_name'];
