...
class Schedule extends CI_Controller {
	...
	public function index(){
		...
		$this->data['posts'] = $this->schedule_model->rs();
		...
		$this->load->view('schedule_list_view',$this->data);
	}
	...
	public function month($yymm=''){
		...
		$this->data['posts'] = $this->schedule_model->search_month($yymm);
		$list_banner = $this->global_model->get_list_banner();
		if (count($list_banner)>0){
			$offset = rand(2,5);
			array_splice($this->data['posts']['data'],$offset,0, array($list_banner));
	...
	public function posts($title=''){
		...
		$this->data['meta']['url'] = SITE_URL . 'schedule/' .$this->data['data']['post_name'];
