...
class Ingredients_model extends CI_Model {
	...
	public function get_rs($type_id = 0)
	{
		...
		$sql = "SELECT {$col} FROM ingredients A LEFT JOIN products B on A.product_id = B.id AND B.product_type = 'ingredients' {$where} ORDER BY sort asc";
	...
	public function get_row($id=0)
	{
		...
		$sql = "SELECT {$col} FROM ingredients A LEFT JOIN products B ON A.product_id = B.id where A.id = '{$id}' {$where}";
		$query = $this->db->query($sql);
	...
	public function get_type_rs(){
		...
		$sql = "SELECT * FROM sub_type WHERE main_type='ingredients' and level = '0' ORDER BY sort asc";
		$query = $this->db->query($sql);
		$result = $query->result_array();
	...
	public function get_filters_rs($parent_id = "0"){
		...
		$sql = "SELECT * FROM sub_type WHERE relation_id = '{$parent_id}' and main_type='ingredients' and level = '1' ORDER BY sort asc";
		$query = $this->db->query($sql);
		$result = $query->result_array();
		return $result;
	}
	...
	public function get_child($parent=0,$active=0){
		...
		$sql = "SELECT * FROM sub_type WHERE main_type='ingredients' and level = '{$active}' and relation_id = '{$parent}' {$where} ORDER BY sort ASC, id DESC";
		$query = $this->db->query($sql);
	...
	public function get_index(){
		$result = array();
		if($this->admin_model){
			$col = "A.*,B.product_no,B.unit_text,B.unit,B.price,B.count";
			$where = "";
		}
		else{
			$col = "A.*,B.product_no,B.unit_text,B.unit,B.price,B.count";
		...
		$sql = "SELECT {$col} FROM ingredients A LEFT JOIN products B on A.product_id = B.id  where index_sort <> '0' {$where} ORDER BY index_sort asc";
		$query = $this->db->query($sql);
		$result = $query->result_array();
		return @$result;
	}
	public function reset_index(){
		if($this->admin_model){
			$sql = "UPDATE ingredients SET index_sort = 0";
			$query = $this->db->query($sql);
		}
	}
}
