...
class Recipes_model extends CI_Model {
	...
	public function get_rs($type_id = 0)
	{
		...
		$sql = "SELECT {$col} FROM recipes A LEFT JOIN products B on A.product_id = B.id AND B.product_type = 'recipes' {$where} ORDER BY sort asc";
	...
	public function get_row($id=0)
	{
		...
		$sql = "SELECT {$col} FROM recipes A LEFT JOIN products B ON A.product_id = B.id where A.id = '{$id}' {$where}";
		$query = $this->db->query($sql);
	...
	public function get_type_rs(){
		...
		$sql = "SELECT * FROM sub_type WHERE main_type='recipes' and level = '0' ORDER BY sort asc";
		$query = $this->db->query($sql);
		$result = $query->result_array();
	...
	public function get_filters_rs($parent_id = "0"){
		...
		$sql = "SELECT * FROM sub_type WHERE relation_id = '{$parent_id}' and main_type='recipes' and level = '1' ORDER BY sort asc";
		$query = $this->db->query($sql);
		$result = $query->result_array();
		return $result;
	}
	...
	public function get_child($parent=0,$active=0){
		...
		$sql = "SELECT * FROM sub_type WHERE main_type='recipes' and level = '{$active}' and relation_id = '{$parent}' {$where} ORDER BY sort ASC, id DESC";
		$query = $this->db->query($sql);
