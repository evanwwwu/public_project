...
<form id="frm" class="pure-form pure-form-stacked">
  ...
  <select name="relation_id">
    ...
    <?PHP foreach ($types as $main): ?>
    <?php if ($main["id"]!=@$row["id"]): ?>
      <option value="<?PHP echo $main["id"]; ?>" level="<?PHP echo $main["level"]; ?>" <?PHP echo (@$row["relation_id"]==$main["id"])?"selected":"" ?>><?PHP echo $main["title"] ?></option>
    <?php endif; ?>
    <?PHP endforeach; ?>
  </select>
  <label>標題</label>
