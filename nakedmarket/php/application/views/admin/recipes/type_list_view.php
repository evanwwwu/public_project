            ...
            <li id="sub_row_<?PHP echo $row["id"]?>">
                ...
                <div class="title_name" style="width:50%;">
                    ...
                    <ul class="level2" style="display:none;">
                        <?PHP foreach ($row["sub"] as $lv2 => $row2): ?>
                        <li id="row_<?PHP echo @$row2["id"]?>">
                            <div>
                                <i value="<?PHP echo @$row2["id"]?>" class="icon-move s2"></i>
                                <input type="checkbox" class="publish" value="<?PHP echo @$row2["id"]?>" <?PHP echo (@$row2["publish"]=="1")?"checked":""?> onclick="list_view.publish($(this),'<?PHP echo ADMIN_URL.$this->data["controller_addr"];?>/type_publish')"/>
                            </div>
                            <div>-------→<?PHP echo $row2["title"];?></div>
                            <div>
                                <a class="pure-button" title="編輯" href="<?PHP echo ADMIN_URL.$this->data["controller_addr"];?>/type_edit/<?PHP echo @$row2["id"]?>"><i class="icon-pencil"></i></a>
                                <a class="pure-button" title="刪除" onclick="list_view.del('<?PHP echo @$row2["id"]?>','<?PHP echo ADMIN_URL.$this->data["controller_addr"]?>/type_del')"><i class="icon-trash"></i></a>
                            </div>
                        </li>
                        <?PHP endforeach; ?>
                    </ul>
    ...
    <script>
        $(function () {
            ...
            $('.level2').sortable({
                ...
                stop: function (event, ui) {
                    var q="";
                    $(this).find('.icon-move.s2').each(function (index, element) {
                        q += '&id[]=' + $(this).attr('value');
                    });
                    $.post('<?PHP echo ADMIN_URL.$this->data["controller_addr"];?>/type_sort', q)
                }
