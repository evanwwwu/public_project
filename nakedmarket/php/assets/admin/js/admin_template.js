            ...
            var methods = {
                'init': function(options) {
                    ...
                    var _add_template = function(btn_instance) {
                        ...
                        var language_index = (btn_instance.parents('.languageTab').index() - 1 < 0) ? 0 : btn_instance.parents('.languageTab').index() - 1,
                        ...
                        site_div = $('<div class="section"></div>'),
                        view_id_input = $('<input type="hidden" name="view_id[]" value="' + view_id + '">'),
                        tem_title = $('<h3 class="acc_btn">樣板 ' + section_count + '<a href="javascript:void(0)" class="del_template" style="float:right;"><i class="icon-trash"></i>DELETE</a></h3>'),
                        csrf_val = "";
                        if(typeof csrf.name != 'undefined') {
                           csrf_val = "&"+csrf.name + '=' + csrf.value;
                        }                
                        $.post(_settings.view_loader_url, 'view_id=' + view_id + '&view=' + view + '&section_idx=' + section_count + '&rand=' + Math.random()+csrf_val, function(data) {
                            var section = $(data);
                            var sdiv = $("<div></div>").append(view_id_input).append(section);
                            site_div.append(tem_title).append(sdiv);
                            template_container.append(site_div);
                            _accordion(template_container);
                            _set_template_input(site_div)
                            btn_instance.show();
                        }, 'html');
                    }
                    ...
                    var _set_template_input = function(section) {
                            ...
                            section.find('.template_img_upload').css('cursor', 'pointer').on('click', function() {
                                var target_img = $(this);
                                var this_img_w = (typeof target_img.attr("upload-width") == "undefined") ? target_img.width() : target_img.attr("upload-width");
                                var this_img_h = (typeof target_img.attr("upload-height") == "undefined") ? "" : target_img.attr("upload-height");
                            ...
                            section.find('a.del_template').on('click', function() {
                                ...
                                //CKEDITOR要一併刪除
                                var textarea = section.find('textarea.ckedit');
                    ...
                    //設定accordion
                    var template_container = $('<div class="my_template_accordion" style="margin:10px auto"></div>');
                    $(this).append(template_container);
                    template_container.append(default_value);
                    _accordion(template_container);
                    _set_template_input(template_container.find('.section'));
                },
                // arr:原POST陣列, columns:DB名稱, languageTab:是否多語系
                'save': function(arr, columns, languageTab) {
                    $('.my_template_accordion').each(function(inx) {
                        ...
                        var lan = (languageTab) ? $(this).parents("div.languageTab").attr("id") : "";
