var uploaded = 0;
var img_url = "http://192.168.0.117/www/cms_sample/";
var edit_view = {
    init: function() {
        ...
        if (typeof($(".date_ui").attr("time")) != "undefined") {
            DateOptions["timeFormat"] = $(".date_ui").attr("time");
        } else {
            DateOptions["showTimepicker"] = false;
        }
        $('.date_ui').datetimepicker(DateOptions);
        $("textarea").autosize();
    },
    menu_click: function() {
        ...
        ckedit: function(obj, css) {
            console.log(css);
            var today = new Date();
            var config = new Object();
            var brand_id = $("select[name=brand_id] option:selected").val()
            config.toolbar = 'Basic';
            config.filebrowserImageUploadUrl = img_url + 'welcome/ckupload/upload/?brand_id=' + brand_id + "&backurl=http://admin.isabelle.com.tw/welcome/cktool";
...
var list_view = {
    ...
    publish: function(obj, addr) {
        ...
        q += '&' + csrf.name + '=' + csrf.value;
        $.post(addr, q, null, 'script');
    },
    del: function(id, addr) {
        if (!confirm('確認刪除?')) return;
        $.post(addr, 'id=' + id+'&'+csrf.name+'='+csrf.value, null, 'script');
    },
    del2:function(id,pid,addr){
        if (!confirm('確認刪除?')) return;
        $.post(addr, 'id=' + id+'&'+csrf.name+'='+csrf.value+'&pid='+pid, null, 'script');
    },
    sort: function(addr) {
        $('tbody').sortable({
            handle: ".icon-move",
    ...
    var project = {
        ...
        table_del:function(obj){        
          if (!confirm('確認刪除?')) return;
          obj.parents(".t_div").remove();
      },
      add_tr:function(obj, type){
        switch(type){
            case "articles":
            var html = '<tr><td><input type="text" name="t_title"> - <input type="text" name="t_text"><a href="javascript:void(0)" onclick="edit_view.delete_tr($(this))">　<i class="icon-trash"></i></a></td></tr>';
            break;
            case "recipes":
            var html = '<tr><td><input type="text" class="pure-input-1-2" name="t_text"><a href="javascript:void(0)" onclick="edit_view.delete_tr($(this))">　<i class="icon-trash"></i></a></td></tr>';
            break;
            case "classes":
            var html = '<tr><td><input type="text" class="pure-input-1-2" name="t_text"><a href="javascript:void(0)" onclick="edit_view.delete_tr($(this))">　<i class="icon-trash"></i></a></td></tr>';
            break;
        }
        obj.parents("table").find("tbody").append(html);
    },
    add_table:function(count,type){
      if($(".t_div").length < count){
        var html ='<div class="t_div"><table class="pure-table pure-table-bordered" width="100%"><thead><tr><th><span>產品規格</span> <button class="del_not del" type="button" onclick="project.table_del($(this))">刪除</button><button class="notice add" type="button" onclick="project.add_tr($(this),\''+type+'\')">新增</button></th></tr></thead><tbody></tbody></table></div>';
        $(".top_content").append(html);
    }
    else{
        if(type!="classes"){
            alert("產品簡介 不可超過"+count+"個");
        }
        else{
            alert("課程簡介 不可超過"+count+"個");            
        }
    }
},
add_classesdate:function(){
    var html = $('<div class="dates"></div>');
    var top = '<input type="hidden" name="date_id[]" value="0">日期:';
    var input = $('<input type="text" name="classes_date[]">');
    var bottom = ' 人數:<input type="text" name="people_limit[]"> 金額:<input type="text" name="price[]"> <a href="javascript:void(0)" onclick="project.delete_date($(this))">刪除<i class="icon-trash"></i></a>';
    ...
    $(".dates_div").append(html.append(top).append(input).append(bottom));
},
delete_date:function(obj){  
  if (!confirm('確認刪除?')) return;
  obj.parent().remove();
},
delete_date2:function(obj,did){ 
  if (!confirm('確認刪除?')) return;
  obj.parent().remove();
  $(".dates_div").append('<input type="hidden" name="d_del[]" value="'+did+'"/>');
