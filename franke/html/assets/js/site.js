	...
	pri_popup_set:function(){
		$('.private_btn').click(function(){
			if ($('.popup.private').hasClass("show")) {
					$('html').css('overflow-y', 'auto');
					$('.xx').removeClass('closed').addClass('opened');
					$('.popup.private').removeClass("show").addClass("notshow");
					if ($('.popup.private').hasClass("notshow")) {
						setTimeout(function() {
							$('..popup.private').removeClass("notshow");
							$('.xx').removeClass('opened');
						}, 1000);
					}
				} else {
					$('.xx').removeClass('opened').addClass('closed');
					$('.popup.private').removeClass("notshow").addClass("show");
					$('html').css('overflow-y', 'hidden');
				};
			});
		$('.popup.private .mask,.popup.private .xx').click(function(){
			//0408
			$('html').css('overflow-y', 'auto');
			$('.xx').removeClass('closed').addClass('opened');
			$('.popup.private').removeClass("show").addClass("notshow");
